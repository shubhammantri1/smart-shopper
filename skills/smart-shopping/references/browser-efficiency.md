...
Before opening browser tabs, form a short, specific plan of which 2-4 sites are actually relevant to this category and request (using the profile's `preferred_marketplaces` if set, or a quick web search to identify the right sites otherwise). Avoid an unscoped "check everything" sweep — it costs the most and adds the least per additional site checked.
